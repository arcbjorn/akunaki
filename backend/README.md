...
A bad signature, tampered body, or unknown connection is an indistinguishable **401** (no cross-connection probing); an unconfigured or non-HMAC provider (Google Health's rotating-key scheme is not yet built) is a **404**. Only the HMAC providers (Oura, Polar) are verifiable here.
