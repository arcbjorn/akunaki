...
The composite owns no formula: `akunaki.application.today_surface` delegates to the recovery and sleep surface services and combines their disclosures, then applies the pure training-label and recommendation rules. Anomaly and ACWR inputs are not yet threaded into this path, so those downshift and rule branches await the anomaly-persistence and load-ingestion units. Verified end to end, including that unshipped blocks never appear as fabricated data.
