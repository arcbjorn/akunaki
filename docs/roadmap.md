...
| **Migrations** | Alembic expand/contract / N−1 rolling on libSQL/Turso | Upgrade/downgrade CI green | **Partial:** revisions through connection lifecycle `0004` upgrade/downgrade/upgrade on local libSQL; head → `0002` → head preserves a legacy job and verifies the `system.noop` backfill; migration tests derive head from the scripts rather than pinning a literal id; expand/contract / N−1 rolling not proven |
