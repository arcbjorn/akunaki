...
1. **Starlette / TestClient:** Dev dependency is **`httpx2==2.10.0`**. Starlette 1.3.1 prefers httpx2 and deprecates plain httpx for `TestClient`. With this pin there is **no** `StarletteDeprecationWarning` from the TestClient path; pytest runs with `filterwarnings=error` so any recurrence would fail the suite.
