...
| Authenticated `/v1` product surface | partial | partial | login, `/v1/session`, `/v1/sleep`, `/v1/recovery`, `/v1/today`, `POST /v1/checkin`, `/v1/tools`, and `/v1/provenance/{token}` reachable. The debug router remains a stand-in to be replaced |
...
| Other detail tables (activity, workouts, labs, temperature, respiratory, …) | no | no | **deliberately deferred**: each arrives with the normalizer that populates it, not as empty tables |
...
| Polar / Google Health fetch clients | partial | partial | **Polar** `PolarFetchClient` (AccessLink exercises) ships and is normalizer-wired; a Polar sync config + OAuth token exchange remain for a live pull. **Google Health** not started |
...
| Product job handlers | yes | yes | `connection.initial_sync`, `raw.normalize`, and `score.recompute` ship, chained: sync enqueues normalize (same commit as the revision), normalize enqueues recompute per affected day |
