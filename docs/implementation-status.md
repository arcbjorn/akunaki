...
| `GET /v1/today` composite surface | yes | yes | authenticated, tenant from session; stitches the recovery score and the sleep summary and mirrors the recovery status at top level. Strain, activity, and the training recommendation **do not ship** in v0.1.0 — absent from the body and disclosed as `*_not_available` gaps, never fabricated; a no-sleep day omits the sleep block (no phantom zero-duration measurement) and gaps are deduplicated. End-to-end tested |
