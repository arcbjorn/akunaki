...
| Windowed feature queries + recovery-input assembly | yes | yes | `daily_sleep_efficiency`, `daily_hrv`, `daily_resting_hr`, and `daily_temperature_deviation` windowed queries (each omits days where the value is unknown), the 42-day `baseline_window_days` helper (target day excluded), and `RecoveryInputService` that builds every present component (adherence direct; efficiency/HRV/RHR/temperature each present only with a mature baseline). Verified against a real DB that a tenant **with** overnight HRV/RHR and a mature baseline reaches a real score, while a sleep-only tenant stays `insufficient`. Respiratory/consistency/prior-load/subjective still await their sources |
| Overnight vitals (HRV, RHR, temperature) detail table + normalizer | yes | yes | migrations `0013`/`0015`; `overnight_vitals` one-to-one detail (at-least-one-of-three invariant); pure `oura_vitals_v0.1.0` normalizer extracts `average_hrv`/`lowest_heart_rate`/`temperature_deviation` (flat or nested `readiness`) from the sleep payload, wake-date-keyed like sleep, naps and empty-signal records skipped, sanity-bounded; flows through the real ingestion loop with shared lineage. **HRV/RHR is what lets the recovery gate pass**; temperature adds a 0.10-weight component |
| Directed z-mapping modes | yes | yes | `Direction` enum with the four exact v0.1.0 forms: `+z` (HRV), `-z` (RHR), `-\|z\|` (temperature: any deviation worse), `-max(z,0)` (respiratory: elevated worse). Golden + mutation tested; respiratory has no source yet |
