...
| Authenticated `/v1` product surface | partial | partial | login, `/v1/session`, `/v1/sleep`, `/v1/recovery`, `/v1/today`, `POST /v1/checkin`, and `/v1/tools` reachable. The debug router remains a stand-in to be replaced |
...
| Score persistence (`daily_health_scores`, `score_factors`) | yes | yes | migration `0014`; versioned like facts (partial unique index on current `(tenant, day, score_code)`; supersede, never rewrite). `ScoreRepository.write_recovery_score` is idempotent by `dependency_hash` (identical recompute is a no-op) and persists signed factor rows (sign from the component's side of the 50 midpoint). Registry + `null-score-iff-insufficient` CHECKs enforced at the DB. **`derivation_run_id` intentionally omitted** (derivation-run subsystem deferred) |
