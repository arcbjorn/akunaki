...
| Windowed feature queries + recovery-input assembly | yes | yes | `daily_sleep_efficiency`, `daily_hrv`, and `daily_resting_hr` windowed queries (each omits days where the value is unknown), the 42-day `baseline_window_days` helper (target day excluded), and `RecoveryInputService` that builds every present component (adherence direct; efficiency/HRV/RHR each present only with a mature baseline). Verified against a real DB that a tenant **with** overnight HRV/RHR and a mature baseline reaches a real score, while a sleep-only tenant stays `insufficient`. Temperature/respiratory/consistency/prior-load/subjective still await their sources |
| Overnight vitals (HRV, resting HR) detail table + normalizer | yes | yes | migration `0013`; `overnight_vitals` one-to-one detail (at-least-one invariant); pure `oura_vitals_v0.1.0` normalizer extracts `average_hrv`/`lowest_heart_rate` from the sleep payload, wake-date-keyed like sleep, naps and empty-signal records skipped, sanity-bounded; flows through the real ingestion loop with shared lineage. **This is what lets the recovery gate pass for a real tenant** |
