...
| Prior-load balance (descriptive ACWR) | yes | yes | pure `prior_load`: ACWR = acute (7-day) / chronic-weekly (28-day / 4) under **strict** coverage (any unknown day → undefined → component omitted); the exact band mapping (under-load toward 0, 0.8-1.3 balance band = 100, over-load to 0 at a≥2.0); the `all_zero_rest` defined-balanced case and zero-chronic/nonzero-acute undefined case. Golden + mutation tested. **Daily strain-load has no source yet** (canonical load needs Polar HR-zone data), so the component is omitted for every current tenant — honestly, verified end to end; a fake fully-covered load source produces a present balanced component |
