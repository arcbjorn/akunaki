...
| Windowed sleep-feature queries + recovery-input assembly | partial | partial | `daily_sleep_efficiency` (total sleep / total in-bed, defined only when **every** session's in-bed is known — else the day is omitted), the 42-day `baseline_window_days` helper (target day excluded), and `RecoveryInputService` that builds the present sleep components (adherence always; efficiency only with a mature baseline). Verified against a real DB that sleep-only tenants assemble to an **`insufficient`** recovery. **HRV/RHR/temperature feature queries need their detail tables + normalizers first**; score persistence and the `/v1/recovery` surface follow |
