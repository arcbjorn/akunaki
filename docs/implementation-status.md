...
| Serve stored scores on read (`/v1/recovery`, `/v1/today`) | yes | yes | `ServedRecoveryService` returns the persisted current score (with its factors, and gaps re-derived from present factor codes via the shared pure rule), falling back to compute-on-read only for a day never scored. Both surfaces read from storage; the compute path stays for the recompute handler. Tested via a fake reader (branching) and end to end (a seeded score is served verbatim over HTTP). **Not yet surfaced: stored version/freshness metadata in the response body** — carried through the service but not exposed |
