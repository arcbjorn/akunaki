...
| Anomaly detectors (Stage 4) | partial | partial | pure `anomalies`: the six enabled v0.1.0 detectors (low HRV, elevated RHR, deviant temp, elevated resp, low activity, short sleep) with exact z-thresholds and moderate/high severity at the clamp; deferred detectors intentionally absent. Golden + mutation tested. **Per-day open/severity only** — the 2-day clear tracking (stateful) and anomaly persistence, plus threading z-scores into the surfaces, are follow-ups |
