...
| `score.recompute` job handler | yes | yes | assembles the recovery surface for a local health day and persists it through the versioned score repository; runs through the **real worker runtime**. A full-history day writes a real score with signed factors; a sleep-only day writes an honest `insufficient` row (a stored outcome, not an absence); redundant recompute is idempotent; a malformed payload dead-letters |
...
| Anomaly detectors + persistence (Stage 4) | partial | partial | pure `anomalies`: the six enabled v0.1.0 detectors (exact z-thresholds, moderate/high at the clamp), direction-aware clear checks, and the **2-consecutive-day clear state machine** (open/update/close with peak-severity retention). Persisted via the `anomalies` interval table (migration `0018`, one active per feature) through `AnomalyRepository` + `AnomalyTracker`. A persisted **high-severity anomaly floors the training label at `light`** — wired into `/v1/today` and verified end to end. Golden + mutation tested. **Not yet running the tracker in the scoring pipeline** (needs per-feature z-scores threaded, same blocker as ACWR); intervals are populated by the tracker but nothing calls it automatically yet |
